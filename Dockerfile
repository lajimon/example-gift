...
CMD java -jar /app.war --spring.datasource.hikari.jdbc-url="jdbc:mysql://${DB_HOST}:${DB_PORT}/gift?serverTimezone=UTC&characterEncoding=UTF-8" \
     --spring.datasource.hikari.username="${DB_USERNAME}" --spring.datasource.hikari.password="${DB_PASSWORD}" \
     --spring.flyway.url="jdbc:mysql://${DB_HOST}:${DB_PORT}/gift?serverTimezone=UTC&characterEncoding=UTF-8" \
